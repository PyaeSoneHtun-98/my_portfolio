...
const ExperienceCard: React.FC<ExperienceCardProps> = ({ company, position, startDate, endDate, description, skills }) => {
  return (
    <div className=" p-4 md:p-6 rounded-lg text-white">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-2xl font-bold">{company}</h2>
        <span className="text-gray-400">{startDate} - {endDate}</span>
      </div>
      <h3 className="text-lg text-indigo-400 mb-2">{position}</h3>
