...
const ProjectCard: React.FC<ProjectCardProps> = ({ imageSrc, title, languages, description, demoLink }) => {
  ...
  return (
    <div className="p-4 rounded-lg shadow-lg w-full bg-zinc-800">
      ...
      <div className="p-4">
        ...
        <div className="flex space-x-2 mt-2">
          {languages.map((language, index) => (
            <span key={index} className="text-indigo-300">{language}</span>
          ))}
        </div>
        <p className="text-gray-400 text-sm md:text-base mt-2">
          {description.length > 100 ? `${description.substring(0, 100)}...` : description}
          {description.length > 100 && (
            <a className="text-indigo-300 cursor-pointer" onClick={handleLearnMoreClick}>
              Learn more &gt;
            </a>
          )}
        </p>
