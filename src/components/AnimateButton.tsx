...
const EncryptButton: React.FC = () => {
  ...
  const scramble = () => {
    ...
    intervalRef.current = window.setInterval(() => {
      const scrambled = langData.resume.split("")
      ...
      if (pos >= langData.resume.length * CYCLES_PER_LETTER) {
        stopScramble();
      }
    }, SHUFFLE_TIME);
  };
  ...
  return (
    <motion.button
      ...
      className="group relative overflow-hidden rounded-lg border-[1px] border-neutral-500 bg-zinc-800 px-4 py-2 font-mono font-medium uppercase text-neutral-300 transition-colors hover:text-indigo-300"
    >
